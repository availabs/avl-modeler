...
//      console.log('BgsHouseholds', BgsHouseholds)
   ...
   Object.keys(personsBgs).map(pid => {
    ...
    useEffect(()=>{
        ...
        return falcor.chunk(['acs',[...uniqueBgs],[2019],[...Object.values(bg_cenvars)]],{chunksize:50})
       ...
       // console.log('falcorCache', falcorCache, Object.values(output))
        ...
        let acsData= selectedBlockGroups.map((geoid, i) => {
           ...
           console.log('bgDataValue', bgDataValue)
            ...
            let hhinc1V = bgDataValue[0].B19001_002E + bgDataValue[0].B19001_003E+bgDataValue[0].B19001_004E+bgDataValue[0].B19001_005E
            ...
            return{  
                ...
                GEOID:geoid, POPBASE:popbaseV > 0 ? popbaseV : 0,
...
// let test = acsData.filter(d => d.GEOID.includes(selectedBlockGroups))
    ...
    let synPopAcsTotals =  ACS_Data.map( d => {
        ...
        let popSynPbase= Object.keys(selectedBgsPersons).map(bg => {
            ...
            return {GEOID:d.GEOID, HHBASE_ACS:d.HHBASE, HHBASE_SynPop:flatten(popSynHHbase).length,
                    POPBASE_ACS:d.POPBASE, PopBase_SybPop:flatten(popSynPbase).length }
    ...
    return (
        ...
        <div className="w-45 bg-gray-600 text-white">
            ...
            <div>
            Number of BGs: {selectedBlockGroups.length.toLocaleString()}
            </div>

            <div>
            Number of SynPop Households: {selectedSynHouseholds.length.toLocaleString()}
            </div>
